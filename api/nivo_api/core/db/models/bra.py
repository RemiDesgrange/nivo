...
from sqlalchemy import Column, TEXT, Integer, DateTime, ForeignKey, CheckConstraint, Boolean, Date
...
Zone = AbstractSpatialTable('bra_zone', metadata,
...
Massif = AbstractSpatialTable("bra_massif", metadata,
                              Column('bm_id', UUID(as_uuid=True), primary_key=True, default=uuid.uuid4),
                              Column('bm_name', TEXT, unique=True, nullable=False),
                              Column('bm_department', UUID(as_uuid=True), ForeignKey('bra_department.bd_id'),
                                     nullable=False),
                              Column('bm_zone', UUID(as_uuid=True), ForeignKey('bra_zone.bz_id'), nullable=False),
                              Column('the_geom', Geometry('POLYGON', srid=4326), nullable=False)
                              )
...
DangerousSlopes = ENUM('NE', 'E', 'SE', 'S', 'SW', 'W', 'NW', 'N', name='dangerous_slopes_t', metadata=metadata)
...
BraSnowRecord = AbstractTable('bra_snow_record', metadata,
                              ...
                              Column('s_bra_record', UUID(as_uuid=True), ForeignKey('bra_record.br_id'),
                                     nullable=False),
                              Column('s_altitude', Integer, CheckConstraint('s_altitude>0'), nullable=False),
                              Column('s_snow_quantity_cm_north', Integer, CheckConstraint('s_snow_quantity_cm_north>0'),
                                     nullable=False),
                              Column('s_snow_quantity_cm_south', Integer, CheckConstraint('s_snow_quantity_cm_south>0'),
                                     nullable=False)
                              )
BraFreshSnowRecord = AbstractTable('bra_fresh_snow_record', metadata,
                                   Column('bfsr_id', UUID(as_uuid=True), primary_key=True, default=uuid.uuid4),
                                   Column('bfsr_bra_record', UUID(as_uuid=True), ForeignKey('bra_record.br_id'),
                                          nullable=False),
...
WeatherForcast = AbstractTable('bra_weather_forcast', metadata,
                               Column('wf_id', UUID(as_uuid=True), primary_key=True, default=uuid.uuid4),
                               Column('wf_bra_record', UUID(as_uuid=True), ForeignKey('bra_record.br_id'),
                                      nullable=False),
                               Column('wf_expected_date', DateTime, nullable=False),
                               Column('wf_wind_altitude', Integer, CheckConstraint('wf_wind_altitude>0'),
                                      nullable=False),
                               ...
                               Column('wf_wind_force', Integer, CheckConstraint('wf_wind_force>0'), nullable=False)
                               )

RiskEvolution = ENUM('UP', 'DOWN', 'STABLE', name='risk_evolution_t', metadata=metadata)

RiskForcast = AbstractTable('bra_risk_forcast', metadata,
